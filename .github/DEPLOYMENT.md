...
## RDS 경량 시드 (Actions 수동)

관리자 테스트 계정은 Deploy와 함께 자동으로 넣지 않는다. Actions의
`Seed RDS` 워크플로를 **수동** 실행한다. EC2에 `psql`/`python3`
추가 설치는 필요 없고, Deploy와 같은 Java 21 + JDBC로 SQL을 적용한다.
좌표·주소가 바뀐 시드 SQL을 RDS에 반영할 때도 같은 워크플로로 재실행한다.

### 사전 조건

1. `Deploy`가 최소 한 번 성공해 EC2에 앱·secret 파일이 있어야 한다.
2. `ChookJiBup_data_pipeline`으로 날짜·위경도가 있는 `festivals`가 충분해야 한다.
...
7. `ADMIN_DEPLOY_PATH`가 실제 EC2 앱 경로와 같아야 한다(예: `demoAdmin_BE`).
...
| 항목 | 값 |
| --- | --- |
| 이메일 | `admin01@seed.mapo.go.kr` |
| 비밀번호 | `qwer1234` |
| 계정 종류 | 공무원 (`GOVERNMENT`) |

비밀번호는 예전 초안의 `SeedPass1!`가 아니다.
