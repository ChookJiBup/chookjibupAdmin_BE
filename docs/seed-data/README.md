...
- `festivals`는 파이프라인 원본이다. 시드 SQL은 연도·상태 계산에 필요한 날짜를 읽기만 하며
  모드(`visitor_count_input_mode`), 운영시간, 설명, 주소를 UPDATE하지 않는다. 방문 시나리오와
  원본 모드가 맞지 않으면 파이프라인 적재 단계에서 정정한 뒤 시드를 실행한다.
...
### GitHub Actions (Seed RDS)

CI(`./gradlew test`)에는 시드를 넣지 않는다. 실 RDS용 시드는 Actions
**Seed RDS**를 수동 실행한다. EC2에 `psql` 설치는 필요 없고
`./gradlew applyRdsSeed`(JDBC)를 사용한다. 재시드도 동일하다.

상세는 `.github/DEPLOYMENT.md`의 **RDS 경량 시드 (Actions 수동)**.
