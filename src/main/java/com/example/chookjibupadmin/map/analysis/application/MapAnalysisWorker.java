...
public class MapAnalysisWorker {
    ...
    void process(MapAnalysisJob job) {
        try {
            ...
            byte[] image=storagePort.read(job.getInputImageKey(),properties.maxInputBytesOrDefault());
            MapAnalysisResult result=analysisPort.analyze(image,map.getAnalysisContentType().getValue(),
                    job.getInputImageWidth(),job.getInputImageHeight());
            resultService.complete(job.getPublicId(),result);
        } catch(MapAnalysisException e) { handleFailure(job,e.code(),e.getMessage(),e.retryable());
        } catch(RuntimeException e) { handleFailure(job,"ANALYSIS_INTERNAL_ERROR","Map analysis failed",true); }
    }
    private void handleFailure(MapAnalysisJob job,String code,String message,boolean retryable) {
        if(retryable&&job.getAttemptCount()<properties.maxAttemptsOrDefault())job.retry(code,message);
        else job.fail(code,message);
        jobService.save(job);
        log.warn("Map analysis job failed: jobId={}, code={}, retryable={}",job.getPublicId(),code,retryable);
    }
}
